...
//Проверка нанесения урона персонажами
...
void firstTest()
{
	...
	test[0].name = "Атаки - ";
	test[1].name = "Регенерация - ";
	test[2].name = "Исцеление - ";
	test[3].name = "Добавление в команду - ";
	test[4].name = "Смерть персонажа - ";

	test[0].value = attacks(Leonid, Andrey);
	test[1].value = regeneration(Leonid, Andrey);
	test[2].value = receivingTreatment(Leonid, Andrey);

	//Просто создание группы персонажей
	...
	for (size_t i = 0; i < test.size(); i++)
		cout << test[i].name << ((test[i].value) ? "Успешно" : "Провально") << endl;
